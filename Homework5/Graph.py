...
class Graph:
    ...
    def load(fileName : str):
        ...
        for node, edges in data.items():
            for edge in edges:
                for end, cost in edge.items():
                    graph.nodes[node].edges.append(Edge(graph.nodes[node], graph.nodes[end], cost))
                
            graph.nodes[node].edges.sort(key=lambda e: (e.cost, e.end.name))
